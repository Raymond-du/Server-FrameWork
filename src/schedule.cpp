...
namespace raymond
{
	...
	FiberSchedule::FiberSchedule(const std::string &name, bool useCurThread,size_t coreNum) 
		...
		if (useCurThread)
		{
			...
			t_schedule_fiber.reset(new Fiber(std::bind(&FiberSchedule::scheduleFunc, this)));
	...
	void FiberSchedule::execSuspendFiber(Fiber::ptr fiber) {
	...
	void FiberSchedule::scheduleFunc()
	{
		...
		//设置线程局部变量		
		setScheduler(this);
		t_schedule_fiber.reset(new Fiber());
		//允许线程hook
		...
		while (true)
		{
			fiber.reset();
			{
				...
				m_conVar.wait(lock, [this] {
					...
					return !this->m_fibers.empty() || (m_joinFlag && m_activeThreadNum.load() <= 0);
				});
			...
			//	}
				...
				fiber->swapIn();
				m_activeThreadNum --;
				//如果fiber 没有执行结束 再次加入到执行队列中&& mjoinFlag == false
				if (fiber->getStatus() == Fiber::READY )
				{
					schedule(fiber);
				} else if (fiber->getStatus() == Fiber::HOLD) {
					m_suspFiberNum++;
				}
			}
		}
