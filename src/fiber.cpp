...
namespace raymond
{
	...
	// 当前执行的协程
	static thread_local Fiber *t_curFiber = nullptr;
	//配置一下stacksize的默认大小
	...
	Fiber::~Fiber()
	{
		...
		if (m_stack)
		{
			...
			RAYMOND_ASSERT2(m_status == TERM || m_status == HOLD || m_status == EXCEPT,std::to_string(m_id).c_str());
